...
half3 LightingPhysicallyBased(BRDFData brdfData, BRDFData brdfDataClearCoat,
    ...
    float HardShadowRecive, float HardShadowReciveThreshold,float SmoothShadowReceiveLowerBound,
    float SmoothShadowReceiveSize,float3 smoothNormalWS)
{
    half NdotL = saturate(dot(smoothNormalWS, lightDirectionWS));
    NdotL = dot(smoothNormalWS, lightDirectionWS);
    float ramp = saturate(((NdotL * 0.5 + 0.5) - rampStart) / rampSize);
    float Step = ramp * rampStepCount;
    half3 radiance;// = lightColor * (lightAttenuation * NdotL);
    if (lightAttenuation >= midLineThreshold - 0.05)
    {
        ramp = (smoothstep(floor(Step), floor(Step) + rampSmooth, Step) + floor(Step)) / rampStepCount;
    }
    ...
    if (Step != floor(Step) && lightAttenuation >= midLineThreshold)
    {
        radiance = lerp(midColor.rgb * lightColor, glossyColor.rgb * lightColor, ramp * lightAttenuation);
    }
    if ((lightAttenuation * NdotL) < rimColorThreshold)
    {
        radiance += lerp((0, 0, 0), rimColor.rgb, pow(1 - max(0, dot(normalWS, viewDirectionWS)), rimColorPower));
    } 
...
half3 LightingPhysicallyBased(BRDFData brdfData, BRDFData brdfDataClearCoat, Light light, half3 normalWS, half3 viewDirectionWS, half clearCoatMask, bool specularHighlightsOff, 
			      ...
			      float SmoothShadowReceiveLowerBound, float SmoothShadowReceiveSize, float3 smoothNormalWS)
{
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData, half4 glossyColor, half4 darkSideColor, float rampStart, 
                            ...
                            float HardShadowRecive, float HardShadowReciveThreshold,float SmoothShadowReceiveLowerBound, float SmoothShadowReceiveSize, float3 smoothNormalWS)
{
    ...
    if (IsMatchingLightLayer(mainLight.layerMask, meshRenderingLayers))
    {
        lightingData.mainLightColor = LightingPhysicallyBased(brdfData, brdfDataClearCoat,
                                                              ...
                                                              HardShadowRecive, HardShadowReciveThreshold,SmoothShadowReceiveLowerBound,SmoothShadowReceiveSize,smoothNormalWS);
    }
